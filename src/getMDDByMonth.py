...
threshold = 0.5  # 自定义的MMD值阈值

for i in range(similarity_matrix.shape[0]):
    if np.all(similarity_matrix[i, :] < threshold):
        source_domain.append(i + 1)
    else:
        target_domain.append(i + 1)
...
# plt.xlabel('Month')
# plt.ylabel('Month')
plt.savefig('building_id_20_Feature_Heatmap.png')
plt.show()
